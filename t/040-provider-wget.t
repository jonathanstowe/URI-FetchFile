...
if $type.is-available {
    lives-ok {
        nok $type.fetch(uri => 'http://example.com/NotThEre', file => 'test-output'), "get with a 404";
    }, "fetch on a non-existent file";
    nok 'test-output'.IO.e, "and the file didn't get created";
    lives-ok {
        ok $type.fetch(uri => 'http://example.com/index.html', file => 'test-output'), "get with a real resource";
